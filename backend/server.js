  ...
  async (req, res) => {
    ...
    try {
      ...
      if (XLSX && XLSX.utils && XLSX.write) {
        ...
        /* ★★★★★ 关键修复：将第3列“Bild/图片”替换为 Excel 的 IMAGE() 公式，以显示网络图片 ★★★★★ */
        try {
          const imageColIndex = 2; // 0-based: 0:#,1:Artikel-Nr.,2:Bild
          const encode = XLSX.utils.encode_cell;
          for (let r = 1; r < rows.length; r++) { // 跳过表头，从第2行开始
            const addr = encode({ r, c: imageColIndex });
            const cell = ws[addr];
            const url = cell && typeof cell.v === "string" ? cell.v : "";
            if (url && /^https?:/i.test(url)) {
              // 兼容引号
              const safe = url.replace(/"/g, '""');
              ws[addr] = { f: `IMAGE("${safe}")` }; // 让新版 Excel 直接渲染图片
            }
          }
        } catch (e) {
          console.warn("[export:xlsx:image-formula] skip:", e?.message || e);
        }
        /* ★★★★★ 修复结束 ★★★★★ */

...
/** 兼容式导入 export.js（无论默认导出/具名导出/CJS 均可） */
...
app.post("/v1/api/export/excel-html", (req, res) => {
  try {
    ...
    const colHtml = columns.map(c => `<th style="border:1px solid #999;padding:4px;background:#eee">${esc(c.title || c.key)}</th>`).join("");
...
app.post("/v1/api/export/table-pdf", (req, res) => {
  try {
    ...
    const scale = pageWidth / (totalWidth || 1);
    const colPx = widths.map(w => Math.max(30, Math.floor(w * scale)));

    let y = doc.y, x = doc.x;
    doc.fontSize(9);
    columns.forEach((c,idx)=>{ doc.rect(x,y,colPx[idx],18).fill("#eee").stroke("#aaa"); doc.fillColor("#000").text(String(c.title||c.key), x+3, y+4, { width: colPx[idx]-6 }); x += colPx[idx]; });
    y += 18;

    const drawRow = (rIdx) => {
      const r = rows[rIdx];
      let localX = doc.x, rowHeight = 16;
      columns.forEach((c,idx)=>{ const txt = String(c.key==="index" ? rIdx+1 : r?.[c.key] ?? ""); const h = doc.heightOfString(txt, { width: colPx[idx]-6 }); rowHeight = Math.max(rowHeight, h+6); });

      if (y + rowHeight > doc.page.height - doc.page.margins.bottom) {
        doc.addPage(); y = doc.y; localX = doc.x;
        columns.forEach((c,idx)=>{ doc.rect(localX,y,colPx[idx],18).fill("#eee").stroke("#aaa"); doc.fillColor("#000").text(String(c.title||c.key), localX+3, y+4, { width: colPx[idx]-6 }); localX += colPx[idx]; });
        y += 18; localX = doc.x;
      }

      localX = doc.x;
      columns.forEach((c,idx)=>{ const val = c.key==="index" ? rIdx+1 : r?.[c.key] ?? ""; doc.rect(localX,y,colPx[idx],rowHeight).stroke("#ddd"); doc.text(String(val), localX+3, y+3, { width: colPx[idx]-6 }); localX += colPx[idx]; });
      y += rowHeight;
    };
